...
1. Discovers issues (Jira board poller: To Do + bot assignee; optional Azure Boards work-item webhooks)
...
Assignment to a name in **`JIRA_TRIGGER_USER`** means the issue is eligible
...
| Compact recap `finish=None` | **Wait.** Last assistant `agent=compaction` / `summary=true` is incomplete (not a crash, not success). Log `finish='stop' summary=None` is the *work* turn; the UI recap is a later message | OpenCode 1.18 lists the recap before `info.finish` is set; treating that as unfinished ERROR'd the job while the operator still saw the summary |
...
- Treat a compact recap (`agent=compaction`, `summary=true`, empty parts, `finish=None`) as a crashed turn *or* as COMPLETE because the UI already shows the summary. Wait for auto-resume. Success only if a later **non-recap** assistant turn is actually done.
- Treat a compact recap (`agent=compaction`, `summary=true`, empty parts, `finish=None`) as a crashed turn *or* as COMPLETE because the UI already shows the summary. Wait for auto-resume. Success only if a later **non-recap** assistant turn is actually done.
...
| CLI offline zip | A second release zip **`yaver-clis-windows-x64-*.zip`**, and **`yaver-clis-linux-x64-*.zip`** on the Linux release. Three installers copy only that CLI plus its host config (`opencode.json`, `config.toml`, `settings.json`). Each installer finds the existing binary, renames it with the date at the end, and puts the new binary in that directory. No agents and no skills. Agents stay on **`install-agents.bat`** / **`install-agents.sh`** in the product zip. Root **`VERSIONS.txt`** records OpenCode **1.18.10**, Codex **0.149.0**, and Claude Code **2.1.280**. CI refuses any other value. |
