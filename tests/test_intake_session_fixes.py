...
def test_adf_comment_splits_newlines():
    ...
    for p in doc["content"]:
        for n in p["content"]:
            if n.get("type") == "text":
                texts.append(n["text"])
                if p.get("type") != "codeBlock":
                    assert "\n" not in n["text"]
    assert texts == ["line1", "line2"]


def test_adf_comment_renders_plan_wiki_as_heading_and_code():
    from src.jira.client import _comment_body_to_adf
    ...
    body = (
        ...
        "* To *revise*: remove {noformat}plan_ready{noformat}\n"
        "----\n"
        "_Plan generated by the planning agent._\n"
    )
    doc = _comment_body_to_adf(body)
    kinds = [n["type"] for n in doc["content"]]
    assert "heading" in kinds
    assert "codeBlock" in kinds
    assert "bulletList" in kinds
    code = next(n for n in doc["content"] if n["type"] == "codeBlock")
    assert code.get("attrs", {}).get("language") == "markdown"
    assert "# Plan" in code["content"][0]["text"]
