"""PromptBuilder: only plan + build paths (system + Jira title + description)."""
...
def test_plan_path_includes_title_and_description():
    ...
    assert "Plan request" in p
    assert "## Role" in p
    assert "Unattended" in p or "unattended" in p.lower()
    assert "## Git policy" not in p
...
def test_build_without_plan_still_has_description():
    """Regression: build must include description even without a plan file."""
    p = PromptBuilder.build_build_prompt(
        "BUG-9",
        "Fix login",
...
def test_only_two_primary_paths():
    """Public API is plan + build; aliases may remain but same content shape."""
    plan = PromptBuilder.build_plan_prompt("K-1", "s", "d")
    build = PromptBuilder.build_build_prompt("K-1", "s", "d")
    assert "Plan request" in plan
    assert "Build request" in build
    assert "Jira title" in plan and "Jira title" in build
    assert "Jira description" in plan and "Jira description" in build


def test_compat_aliases_map_to_two_paths():
    """Legacy names still work and include title + description."""
