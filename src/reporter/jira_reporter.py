...
"""
    ...
    def append_plan_to_description(self, state: JiraAgentState, plan_content: str) -> bool:
        ...
        block = (
            ...
            f"_Mode: plan — no GitLab push. Change Mode to_ `build` "
            f"_in {{params}} (or open a new Mode: build issue) to implement._\n"
        )
        try:
...
* To *implement*: change {{noformat}}Mode: plan{{noformat}} to {{noformat}}Mode: build{{noformat}} in {{noformat}}{{params}}{{noformat}} and put the ticket on *To Do*, *or* open a *new* {{noformat}}Mode: build{{noformat}} issue (same Repository / branches)
