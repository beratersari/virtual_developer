...
"""
    ...
    def post_plan_summary(self, state: JiraAgentState, plan_content: str) -> Optional[str]:
        ...
        if is_azure_work_item_key(getattr(state, "issue_key", "") or ""):
            next_steps = (
                "* Review the plan in this comment\n"
                "* To *revise*: comment `/planRefactor <what to change>` and mention me\n"
                "* To *implement*: comment `/planExecute` and mention me\n"
                "* Or open a *new* {{noformat}}Mode: build{{noformat}} issue "
            ...
            next_steps = (
                "* Review the plan in this comment\n"
                "* To *revise*: remove {{noformat}}plan_ready{{noformat}}, add "
                "{{noformat}}plan_refactor{{noformat}}, and comment tagging the bot\n"
                "* To *implement*: rename {{noformat}}plan_ready{{noformat}} → "
                "{{noformat}}plan_execute{{noformat}} while the ticket is "
                "*In Progress* (Mode in {{params}} can stay {{noformat}}plan{{noformat}})\n"
                "* Or open a *new* {{noformat}}Mode: build{{noformat}} issue "
                "(same Repository / branches)\n"
                "* No dashboard Start button"
            )
