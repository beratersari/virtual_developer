...
"""
    ...
    def append_plan_to_description(self, state: JiraAgentState, plan_content: str) -> bool:
        ...
        block = (
            ...
            f"_Mode: plan — no GitLab push. Set_ `Mode: build` _and re-queue (or Start plan) to execute._\n"
        )
        try:
...
* *Start build (pick one):*
** Set {{noformat}}Mode: build{{noformat}} in {{noformat}}{{params}}{{noformat}} and re-queue from To Do
** Ops dashboard → open this issue → *Start plan*
** Add label {{noformat}}ai-start-work{{noformat}} (board poller picks it up)
** Enable *Auto-start plans* in dashboard settings
