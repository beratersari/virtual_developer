...
* To *revise*: remove {{noformat}}plan_ready{{noformat}}, add {{noformat}}plan_refactor{{noformat}}, and comment tagging the bot
* To *implement*: rename {{noformat}}plan_ready{{noformat}} → {{noformat}}plan_execute{{noformat}} while the ticket is *In Progress* (Mode in {{params}} can stay {{noformat}}plan{{noformat}})
* Or open a *new* {{noformat}}Mode: build{{noformat}} issue (same Repository / branches)
...
"""
        ...
        try:
            ...
            if hasattr(self.client, "add_labels"):
                self.client.add_labels(state.issue_key, [PLAN_READY_LABEL])
        except Exception as e:
