...
"""
    ...
    def append_plan_to_description(self, state: JiraAgentState, plan_content: str) -> bool:
        ...
        block = (
            ...
            f"_Mode: plan — no GitLab push. Set_ `Mode: build` _and move the issue back to To Do to execute._\n"
        )
        try:
...
* To *start build*: set {{noformat}}Mode: build{{noformat}} in the description ({{noformat}}{{params}}{{noformat}} block) and move this issue back to *To Do*
* The board poller will pick it up and run execution (no dashboard Start button)
